...
# ══════════════════════════════════════════════════════════════════════════════
# PREMIUM UI ANIMATIONS & SEBI SHIELD CSS
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
.legal-modal {
    ...
    box-shadow: 0 10px 40px rgba(255, 76, 76, 0.2);
}
...
.ticker-bar { background:rgba(10, 14, 30, 0.7); backdrop-filter: blur(10px); border-bottom:1px solid rgba(15, 26, 53, 0.5); padding:0 20px; display:flex; align-items:center; position:sticky; top:0; z-index:999; height:42px; overflow-x: auto; white-space: nowrap; scrollbar-width: none; }
.ticker-bar::-webkit-scrollbar { display: none; }
.ticker-item { display:flex; align-items:center; gap:8px; padding:0 18px; border-right:1px solid rgba(15, 26, 53, 0.5); height:100%; }
...
.stTabs [data-baseweb="tab-list"] { background:transparent; border-bottom:1px solid rgba(15, 26, 53, 0.5); padding:0 20px; gap:8px; }
...
.sec-hdr { display:flex; align-items:center; gap:10px; padding:18px 0 12px 0; border-bottom:1px solid rgba(10, 16, 32, 0.5); margin-bottom:16px; }
...
.mood-banner { display:flex; align-items:stretch; background:rgba(13, 17, 32, 0.6); backdrop-filter: blur(5px); border:1px solid rgba(15, 26, 53, 0.8); border-radius:10px; overflow:hidden; margin:12px 0 16px; }
...
.pick-card { background:rgba(13, 17, 32, 0.8); backdrop-filter: blur(5px); border:1px solid rgba(30, 45, 71, 0.6); border-radius:12px; overflow:hidden; margin-bottom:16px; transition: transform 0.25s ease, border-color 0.25s ease, box-shadow 0.25s ease; box-shadow: 0 4px 16px rgba(0,0,0,0.25); }
.pick-card:hover { border-color:#3B7DFB; transform: translateY(-3px); box-shadow: 0 8px 24px rgba(59, 125, 251, 0.15); }
.pick-card-head { display:flex; align-items:center; justify-content:space-between; padding:14px 16px; border-bottom:1px solid rgba(15, 26, 53, 0.5); background:linear-gradient(180deg, rgba(16, 22, 40, 0.9), rgba(13, 17, 32, 0.9)); }
...
.pick-buy-badge { animation: pulseGlow 2s infinite; background:rgba(0, 214, 143, 0.12); color:#00D68F; font-size:10px; font-weight:800; padding:4px 10px; border-radius:5px; border:1px solid rgba(0, 214, 143, 0.3); letter-spacing:.08em; }
.pick-grid { display:grid; grid-template-columns:1fr 1fr; gap:1px; background:rgba(15, 26, 53, 0.5); }
.pick-cell { background:rgba(10, 14, 30, 0.8); padding:10px 14px; }
...
.pick-foot { display:flex; align-items:center; justify-content:space-between; padding:10px 14px; border-top:1px solid rgba(15, 26, 53, 0.5); background:rgba(10, 14, 30, 0.9); }
.pick-rr { font-size:11px; font-weight:700; }
.pick-meta { display:flex; gap:10px; font-size:10px; color:#475569; }

.stDataFrame { border:1px solid rgba(15, 26, 53, 0.6); border-radius:8px; overflow:hidden; }

.sq-grid { display:grid; grid-template-columns:1fr 1fr; gap:12px; }
.sq-card { border-radius:10px; padding:16px; min-height:110px; background:rgba(13, 17, 32, 0.6); backdrop-filter: blur(5px); transition: transform 0.2s; }
.sq-card:hover { transform: translateY(-2px); }
.sq-leading { background:linear-gradient(180deg, rgba(5, 20, 9, 0.8), rgba(13, 17, 32, 0.6)); border:1px solid rgba(0, 214, 143, 0.25); }
.sq-improving { background:linear-gradient(180deg, rgba(7, 16, 10, 0.8), rgba(13, 17, 32, 0.6)); border:1px solid rgba(6, 182, 212, 0.25); }
.sq-weakening { background:linear-gradient(180deg, rgba(18, 14, 5, 0.8), rgba(13, 17, 32, 0.6)); border:1px solid rgba(255, 176, 32, 0.25); }
.sq-lagging { background:linear-gradient(180deg, rgba(18, 5, 5, 0.8), rgba(13, 17, 32, 0.6)); border:1px solid rgba(255, 76, 76, 0.25); }
...
.sec-stat-box { background:rgba(10, 14, 30, 0.6); border:1px solid rgba(15, 26, 53, 0.6); border-radius:8px; padding:12px 16px; text-align:center; }
...
.ig-cta { background:rgba(13, 17, 32, 0.6); border:1px solid rgba(15, 26, 53, 0.6); border-radius:10px; padding:24px; text-align:center; margin:20px 0; }
...
with tab2:
    ...
    with qr:
        ...
        fig_s.update_layout(
            ...
            plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="#475569",
            height=290, margin=dict(l=0, r=0, t=30, b=0),
            yaxis=dict(gridcolor="rgba(15, 26, 53, 0.5)"), xaxis=dict(gridcolor="rgba(15, 26, 53, 0.5)", tickangle=-45)
        )
...
st.markdown(f"""
<div style='text-align:center;padding:24px 20px;border-top:1px solid rgba(15, 26, 53, 0.5);margin-top:24px;'>
  <p style='color:#64748B;font-size:10px;margin:0;font-family:"JetBrains Mono",monospace !important;letter-spacing:.04em;'>
    ...
    <a href='https://instagram.com/momentumfrenzy' style='color:#3B7DFB;text-decoration:none;'>@momentumfrenzy</a>
  </p>
</div>""", unsafe_allow_html=True)
