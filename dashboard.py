...
with tab1:
    ...
    # Example: Loading scanner_data.csv for the live view
...
with tab2:
    ...
    # 1. Load the old results (performance_history.json)
    ...
    if history_df.empty:
        ...
        # Checks for '100' or '100/100' in a column named 'Score' (case-insensitive fallback)
        score_col = next((col for col in history_df.columns if col.lower() == 'score'), None)
        if score_col:
            # Filter rows where the score contains '100'
        ...
        for idx, row in history_df.iterrows():
            # Dynamically extract prices (handling different capitalization of your old JSON keys)
            entry = float(row.get("Entry", row.get("entry", 0)))
            sl = float(row.get("SL", row.get("Stoploss", row.get("sl", entry * 0.98))))
            target = float(row.get("Target", row.get("target", entry * 1.04)))
            
            # Find Exit Price (default to entry if not closed yet)
            actual_exit = float(row.get("Exit Price", row.get("Exit", row.get("exit", entry))))
            
            # Find Lot Size (default to 1 if it's equity)
            contract_lot = int(row.get("Lot Size", row.get("lot_size", 1)))
