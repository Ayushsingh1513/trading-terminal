...
def load_data():
    if not os.path.exists("market_data.json") or not os.path.exists("scanner_data.csv") or not os.path.exists("sector_data.csv"): return None, None, None
    try: return json.load(open("market_data.json")), pd.read_csv("scanner_data.csv"), pd.read_csv("sector_data.csv")
...
st.markdown("""<style>
.stApp{background:#07091A !important; color:#CBD5E1;} header[data-testid="stHeader"],#MainMenu,footer{display:none;}
...
.pick-lbl{font-size:9px; color:#64748B; text-transform:uppercase;} .pick-val{font-size:14px; font-weight:700;}
...
<div class="pick-head">
<div><div class="pick-stock">{pk['Stock']}</div><div style="font-size:11px; color:#94A3B8;">{pk['MTF']} · Weekly: {pk['WeeklyTrend']}</div></div>
...
<span>R:R = 1:{pk['RR']}</span><span>{pk['Setup']}</span>
</div>
</div>""", unsafe_allow_html=True)
else:
...
clean_table = scanner_df[["Stock", "Signal", "MTF", "WeeklyTrend", "Setup", "Price", "Score", "RSI", "VolSurge"]]
