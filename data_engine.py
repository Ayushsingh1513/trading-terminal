...
# Broker-approved MTF (Margin Trading Facility) List
...
def get_market_data():
    ...
    # Safe PCR Fallback (Render cloud IPs usually get blocked by NSE)
    pcr_value = 1.05 
...
def scan_hybrid_setups(sector_trends, mkt):
    ...
    for ticker in STOCK_UNIVERSE:
        try:
            ...
            # MTF & Weekly Filter
            if not is_weekly_uptrend: score = min(score, 49) # Block BUY signal if weekly is bearish
            if mkt['pcr'] > 1.5: score = min(score, 79) # Prevent fresh BUYS in overbought market
...
def track_targets_and_notify(scanner_df, mkt):
    ...
    # Send Market Pulse at Open
    c_time = datetime.now(IST)
    if c_time.hour == 9 and 15 <= c_time.minute <= 30:
        overall_flow = "Mixed" # Placeholder for aggregated flow
